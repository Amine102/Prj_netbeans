 ...
 * @author BLHA
 */
public class Modif_UIController implements Initializable {

    @FXML
    private TextField box_nom_prod;

    @FXML
    private Button btn_modif_image;

    @FXML
    private TextField box_enchère_dep;

    @FXML
    private Button btn_confirm_modif;
    ...
    private void enableModif() {
        box_nom_prod.textProperty().addListener((ObservableValue<? extends String> observable, String oldValue, String newValue) -> {
            ...
            btn_confirm_modif.setDisable(isEnabled);
        });
    ...
    @FXML
    private void modif_produit(ActionEvent event) throws SQLException, Exception {
        ...
        Stage stage = (Stage) btn_confirm_modif.getScene().getWindow();
        stage.close();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        box_nom_prod.setText(Interface_utilController.getList_produit().getTitre());
        System.out.println("TiTRE: " + Interface_utilController.getList_produit().getTitre());
        box_enchère_dep.setText(Interface_utilController.getList_produit().getEnchere_dep());
        System.out.println(FXMLController.getId_util().replaceAll("\\s", "") + "_" + Interface_utilController.getList_produit().getId_produit());
        File file = new File("Image/" + FXMLController.getId_util().replaceAll("\\s", "") + "_" + Interface_utilController.getList_produit().getId_produit().replaceAll("\\s", "") + ".jpg");
        if (file.exists()) {
            System.out.println(file.getAbsolutePath());
            if (file != null) {
                Image image = new Image(file.toURI().toString());
                _image.setImage(image);
                text_image.setText(FXMLController.getId_util().replaceAll("\\s", "") + "_" + Interface_utilController.getList_produit().getId_produit().replaceAll("\\s","")+".jpg");
            }

        } else {
            File file2 = new File("Image/" + FXMLController.getId_util().replaceAll("\\s", "") + "_" + Interface_utilController.getList_produit().getId_produit().replaceAll("\\s", "") + ".png");
            if (file2 != null) {
                Image image = new Image(file2.toURI().toString());
                _image.setImage(image);
                text_image.setText(FXMLController.getId_util().replaceAll("\\s", "") + "_" + Interface_utilController.getList_produit().getId_produit().replaceAll("\\s", "")+".png");
            } else {
                _image.setImage(null);

            }
            btn_confirm_modif.setDisable(true);
            enableModif();
